...
## UPDATE (P5 SUB #R31/R32 + attempt 20-21)

**The MTU bug was actually a phone-side bond mismatch, not a firmware-side MTU bug.** The `clear_ble_bond` workaround (SUB #R31/R32) confirmed this:

- Added `DebugActionActivity` in `app/.../debug/` (manifest + .kt)
  that dispatches on `--es action` extra; current arm is
  `clear_ble_bond` which calls `BluetoothDevice.removeBond(mac)`
  via reflection. Logcat tag `DebugActionAct` shows
  `action='...' ok=...`; `ok=true` means the bond is gone.
- Added dispatch step in `test-harness/harness/orchestrator.py`
  `_dispatch_firmware_update` that calls the Activity via
  `am start` with the BLE MAC from `[controller].ble_mac` in
  `test-harness/config.toml`.
- Manual smoke: `adb shell am start -n
  de.aarondietz.beetmeister/.debug.DebugActionActivity -a
  de.aarondietz.beetmeister.debug.action.RUN --es action
  clear_ble_bond --es mac 3C:0F:02:D2:0F:6A`.
- After the fix, attempt 20 showed `onMtuChanged status=0 mtu=247`
  in the Android logcat (was `status=133 mtu=23` before). The
  controller serial also showed `gap_event MTU handle=1 mtu=247`
  (was absent before — NimBLE never delivered the event).

**Root cause of the original "MTU bug":** `flash_old` wipes the
controller's NVS (the firmware-side bond store), so the phone's
stored link key is stale. The next BLE auto-connect tries to
encrypt with it and gets HCI_ERR_KEY_MISSING, which makes the
link drop and stalls the test. The `pm clear
com.android.bluetooth` and `cmd bluetooth_manager remove-bond`
workarounds were no-ops (the bond is re-loaded from
`/data/misc/bluetooth/` on next Bluetooth app start; the
`bluetooth_manager` shell command was removed in Android 16).
Only `BluetoothDevice.removeBond()` (the `@hide @SystemApi`)
actually clears the persistent bond store, and only the app
itself can call it.

**New blocker discovered (attempt 21):** with the bond cleared,
the BLE connection succeeds (MTU 247, services discovered), but
the app's `BeetGattSession.openGatt` fires a reconnect ~552 ms
after `phase=Connected`. The gate's `LaunchedEffect` (which
delays 900 ms before hiding the gate) is cancelled by the
`Connected → Connecting` phase change. The gate never hides.
The `tapScan` `waitUntil` then times out at 30 s because
`postConnectVisible()` (which checks for the nav item) returns
false — the gate is still visible.

The reconnect is triggered by a `onCharacteristicChanged
uuid=8f2a0005-... size=103` notification from the controller
at `16:35:34.008`, 709 ms after `phase=Connected`. The v0.3.0
firmware's characteristic value pattern apparently triggers
the app's resetSyncState + reconnect path. The current (HEAD)
firmware does not trigger this path during a normal connect.

**This is a v0.3.0 firmware BLE instability, not a test-side
issue.** The test harness is working correctly. The fix is
either:
1. Firmware-side: make the v0.3.0 firmware's BLE characteristic
   stream not trigger the app's resetSyncState + reconnect path
   during the initial connect, OR
2. App-side: make the gate's `LaunchedEffect` resilient to rapid
   `Connected → Connecting → Connected` transitions (e.g. track
   the "stable Connected" state in a separate `MutableState` that
   only resets after a debounce window, instead of using
   `state.connection.phase` as the effect key).

**Manual smoke command for `clear_ble_bond`:**
```
adb shell am start \
...
**Logcat for manual smoke:**
```
adb logcat -d -s DebugActionAct:I
# → DebugActionAct: action='clear_ble_bond' ok=true; finishing
```

**Handover commits for the `clear_ble_bond` fix:**
- `16c9d24` SUB #R31: clear phone-side BLE bond in firmware_update dispatch
- `1b1c37a` SUB #R32: clear BLE bond via app's ClearBleBondReceiver
- `19bf8fb` SUB #R32: switch ClearBleBond from receiver to activity
- `b0819d9` SUB #R32: generalize ClearBleBond to DebugActionActivity

**Test-side commit for the post-fix attempt:**
- `56460b2` SUB #R33: revert postConnectVisible to hasText matcher
  (the hasTestTag approach in R30 didn't survive the BLE
  auto-connect window; hasText is more permissive and matches
  the merged NavigationBarItem parent's text)
