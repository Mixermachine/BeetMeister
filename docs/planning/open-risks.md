...
| `R-002` | Relay board input polarity or threshold may differ from the active-low baseline. | Missed relay activation, unexpected boot-time energizing, or inconsistent switching. | Bench-test the chosen relay board across 3.10 to 3.60 V controller conditions and confirm the active-low input behavior with safe inactive pull-up bias. |
