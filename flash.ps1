...
$proc = Start-Process -NoNewWindow -Wait -PassThru -RedirectStandardOutput stdout.txt -RedirectStandardError stderr.txt -FilePath C:\Espressif\tools\python\v6.0\venv\Scripts\python.exe -ArgumentList @("C:\esp\v6.0\esp-idf\tools\idf.py", "-p", "COM6", "flash")
Write-Host "EXIT: $($proc.ExitCode)"
$stdout = Get-Content stdout.txt -Raw
if ($stdout) { ($stdout -split "`n")[-5..-1] | Write-Host }
$stderr = Get-Content stderr.txt -Raw
if ($stderr) { ($stderr -split "`n") | Select-Object -Last 5 | Write-Host }
